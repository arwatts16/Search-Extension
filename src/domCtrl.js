  ...
  if (msg.message == "sent data") {
    ...
    for (var i = 0; i < dataArr.length && msg.allActive === "true"; i++) {
      if (dataArr[i].active === "true") {
        bits.search.domCtrl.prototype.applyMatches(msg.match, dataArr[i]);
      }
    }
  }
});
...
bits.search.domCtrl.prototype.applyMatches = function(match, searchRes) {
  ...
  for (var i = 0; i < searchRes.data.length; i++) {
    searchRes.data[i].name = searchRes.data[i].name.replace(
      /[^A-Za-z0-9_ ]/g,
      ""
    );
    if (matcher.findMatches(match, body, searchRes.data[i])) {
    ...
    for (var i = 0; i < foundData.length; i++) {
      ...
      if (re.test(textNode.data)) {
        ...
        textNode.data =
          ...
          textNode.data.slice(ind + foundData[i].name.length);
  ...
  for (var i = 0; i < matches.length; i++) {
    matches[i].onclick = function(e) {
      ...
      var nameText = popup.document.createTextNode(
        "Name: " + foundData[e.srcElement.id].name
      );
      name.appendChild(nameText);
      popup.document.body.appendChild(name);
      var type = popup.document.createElement("h3");
      var typeText = popup.document.createTextNode(
        "Type: " + foundData[e.srcElement.id].type
      );
      type.appendChild(typeText);
      popup.document.body.appendChild(type);
      var subtype = popup.document.createElement("h3");
      var subtypeText = popup.document.createTextNode(
        "Subtype: " + foundData[e.srcElement.id].subtype
      );
      subtype.appendChild(subtypeText);
      popup.document.body.appendChild(subtype);
      var source = popup.document.createElement("h3");
      var sourceText = popup.document.createTextNode(
        "Source: " + foundData[e.srcElement.id].source
      );
