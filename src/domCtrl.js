...
bits.search.domCtrl.prototype.findMatches = function(found, wholeData) {
  ...
  for (var i = 0; i < wholeData.data.length; i++) {
    wholeData.data[i].name = wholeData.data[i].name.replace(
      /[^A-Za-z0-9_ ]/g,
      ""
    );
    ...
    for (var i = 0; i < found.length; i++) {
      ...
      if (re.test(textNode.data)) {
        ...
        textNode.data =
          ...
          textNode.data.slice(ind + found[i].name.length);
...
>>>>>>> 489eb22634eefa18bbf0823d53e83153bf48891d
      ...
      var typeText = popup.document.createTextNode(
        "Type: " + found[e.srcElement.id].type
      );
      ...
      var subtypeText = popup.document.createTextNode(
        "Subtype: " + found[e.srcElement.id].subtype
      );
      ...
      var sourceText = popup.document.createTextNode(
        "Source: " + found[e.srcElement.id].source
      );
